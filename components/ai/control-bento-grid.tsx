...
export function ControlBentoGrid() {
  const { deviceData, sendCommand } = useDeviceData()
        ...
        {devices.map((device) => (
          <motion.div
            ...
            className={cn(
              ...
              "bg-white/80 backdrop-blur-xl hover:shadow-md",
              device.state ? "border-gray-200" : "border-gray-100"
            )}
          >
            <div className="flex flex-col justify-between h-full gap-4">
              <div className="flex items-start justify-between">
                <div className={cn(
                  ...
                  device.state ? device.bg : "bg-gray-100"
                )}>
                  <device.icon 
                    size={24} 
                    className={cn(
                      "transition-colors duration-300",
                      device.state ? device.color : "text-gray-400",
              ...
              <div>
                <div className="font-bold text-gray-900 text-lg mb-1">{device.name}</div>
                <div className="flex items-center gap-2">
                   <div className={cn(
                     "size-2 rounded-full",
                     device.state ? "bg-green-500" : "bg-gray-300"
                   )} />
